...
4. Consultar estadísticas de ventas reales:
   ...
   Total ventas exitosas = GREATEST((a), (b))  # IMPORTANTE: NO sumar con +, usar GREATEST para no duplicar pedidos completados con prendas vendidas
...
def get_vendedor_publico(identifier: str, db: Session = Depends(get_db)):
    ...
    # 2. Conteo de ventas exitosas (usar GREATEST para evitar duplicar pedidos completados con prendas vendidas)
    ventas_query = text("""
        SELECT 
          GREATEST(
